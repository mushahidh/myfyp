      ...
      <div class="row-fluid">
        <div class="span7">
          <div class="box paint color_4">
            <div class="title">
              <h4> <span>Edit Field</span> <span style="font-size:11px;">(Fields marked with * are required)</span> </h4>
            </div>
            <div class="content">
              <form class="form-horizontal cmxform" id="validateForm" method="get" action="../../field_controller" onsubmit="javaScript: return validate();" autocomplete="off">
              <%
        				fieldDAO UD=new fieldDAO();
     	 				String idd=request.getParameter("field_id");
     					int id=Integer.parseInt(idd);
						field f=UD.getFieldInfo(id);
        		 %>
                 <div class="form-row control-group row-fluid">
                  <label class="control-label span3" for="with-tooltip">Field Name*</label>
                  <div class="controls span7">
                    <input type="text" id="with-tooltip" name="field_name" value="<%=f.getField_name()%>"  placeholder="Enter Field Name..." class="row-fluid" required onkeyup="javaScript:validate_field_name();">
                     <input type="hidden" name="field_id" value="<%=f.getField_id()%>"  >
                     <label id="field_name_info"  style="color:rgba(255,255,255,0.6);font-size:13px">
  					  </label>
                  </div>
                </div>
                    <div class="form-row control-group row-fluid">
                  <label class="control-label span3" for="default-select">Select FieldStandard</label>
                  <div class="controls span7">
                    <select data-placeholder="Select Grade..." class="chzn-select" id="default-select" name="field_standard">
                      <option value="<%=f.getField_standard()%>"><%=f.getField_standard()%></option>
                   ...
                   <div class="form-row control-group row-fluid">
                  <label class="control-label span3" for="with-tooltip">Field Description</label>
                  <div class="controls span7">
                    <input type="text" id="with-tooltip" name="field_desc" value="<%=f.getField_desc()%>"  placeholder="Enter Field Description..." class="row-fluid" >
                  </div>
                </div>
...
function validate_field_name(){
	 ...
	 var TCode = document.getElementById('field_name').value;
	    if( /[^a-zA-Z1-9 _-]/.test( TCode ) ) {
	    	
	    	field_name_info.innerHTML="Please Enter Only Alphanumeric or _,-,' '  ";
	       return false;
	    }
	    field_name_info.innerHTML=" ";
	    return true;  
