      ...
      <div class="row-fluid">
        <div class="span7">
          <div class="box paint color_24">
            <div class="title">
              <h4> <i class="icon-book"></i><span>Edit Affect</span>  <span style="font-size:11px;">(Fields marked with * are required)</span> </h4>
            </div>
            <div class="content">
              <form class="form-horizontal cmxform" id="validateForm" onsubmit="javaScript: return validate();" action="../../affect_controller">
               
              <%
        				affectDAO AD=new affectDAO();
						
     	 				String course_idd=request.getParameter("course_id");
     					int course_id=Integer.parseInt(course_idd);
     					String field_idd=request.getParameter("field_id");
     					int field_id=Integer.parseInt(field_idd);
						
						courseDAO CDD=new courseDAO();
						course c=CDD.getCourseInfo(course_id);
						
						fieldDAO FD=new fieldDAO();
						field f=FD.getFieldInfo(field_id);
						affect a=AD.getAffectInfo(field_id, course_id);
               ...
               <div class="form-row control-group row-fluid">
                  ...
                  <div class="controls span7">
                      <input type="text" disabled value="<%=f.getField_name() %>" class="row-fluid"/>
                       <input type="hidden" value="<%=a.getField_id() %>" name="field_id" />
                  </div>
                </div>
                
                <div class="form-row control-group row-fluid">
                  <label class="control-label span3">Course</label>
                  <div class="controls span7">
                      <input type="text" disabled value="<%=c.getCourse_name() %>" class="row-fluid"/>
                       <input type="hidden" value="<%=a.getCourse_id() %>" name="course_id" />
                  </div>
                </div>
                     <div class="form-row control-group row-fluid">
                  <label class="control-label span3" for="with-tooltip">Weight*</label>
                  <div class="controls span7">
                    <input type="text" id="weight_id" name="weight" value="<%=a.getWeight() %>" placeholder="Enter weight..." class="row-fluid" required onkeyup="javaScript:validate_weight();">
                 <label id="weight_info"  style="color:rgba(255,255,255,0.6);font-size:13px"></label>
                  </div>
                </div>
 ...
 $(document).ready(function () {
	...
	function validate_course(){
		 ...
		 if(course.selectedIndex==0){
			...
			course_info.innerHTML="Please select a course";
			return false;
	...
	function validate_weight(){
		 ...
		 if(weight==""){
			...
			weight_info.innerHTML="Please associate a weight";

		 }
		 else
		 	{
		 		weight_info.innerHTML="";
		 	}
		 var TCode = document.getElementById('weight_id').value;
		    if( /[^1-9]/.test( TCode ) ) {
		    	
		    	weight_info.innerHTML="Please Enter Only numeric values";
		       return false;
		    }
		    weight_info.innerHTML=" ";
		    return true;  
	...
	function validate()
	{
		if(validate_field()==true&&validate_course()==true&&validate_weight()==true)
			{
			 	return true;
			}
		return false;
